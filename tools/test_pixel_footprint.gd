	...
	for depth in [2.0,20.0,100.0]:
		for width in [2.0,4.0]:
			for mode in [0,2]:
				...
				data.set_instance_custom_data(0,Color(0,1,0,0.25))
				material.set_shader_parameter("pixel_size",width)
				material.set_shader_parameter("coverage_mode",mode)
				await process_frame
				...
				var expected:=int(width*width*(0.25 if mode==2 else 1.0))
				valid=valid and count==expected
				print("PIXEL_TEST depth=",depth," width=",width," coverage_mode=",mode," pixels=",count," expected=",expected)
